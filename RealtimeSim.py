...
from RealtimeSimStrategy import RealtimeSimStrategy, ActionSimStrategy
...
class RealtimeSim:
    ...
    @classmethod
    def sim_model_pred_onemin_action(cls, pred, pt_ratio, lc_ratio, amount, ltp, ac:RealtimeSimAccount, ohlc:OneMinData):
        ad = ActionSimStrategy.model_prediction_onemin(pred, lc_ratio, amount, ac, ltp)
        pt_ratio = int(round(ac.holding_price * (1.0 + pt_ratio))) if ac.holding_side == 'Buy' else int(round(ac.holding_price * (1.0 - pt_ratio)))
        for n in range(len(ad.action)):
            if ad.action[n] == 'entry':
                if ad.order_side[n] == 'Market':
                    ac.entry_order(ad.order_side[n], 0, ad.order_size[n], 'Market', ltp, datetime.now())
                elif ad.order_side[n] == 'Limit':
                    ac.entry_order(ad.order_side[n], TickData.get_bid() if ad.order_side[n] == 'Buy' else TickData.get_ask(), ad.order_size[n], 'Limit', ltp, datetime.now())
            elif ad.action[n] == 'cancel':
                ac.cancel_all_orders()
        ac.check_execution(ltp, datetime.now(), ohlc)
        ac.update_pnl(ltp)
        return ac
    ...
    @classmethod
    def sim_model_pred_onemin_avert_limit_entry(cls, pred, pt_ratio, lc_ratio, amount, ltp, ac:RealtimeSimAccount, ac2:RealtimeSimAccount, avert_period_kijun, avert_val_kijun, ohlc:OneMinData):
        ac2 = cls.sim_model_pred_onemin_limit_entry(pred, pt_ratio, lc_ratio, amount, ltp, ac2, ohlc)
        if len(ac2.total_pl_log) > avert_period_kijun:  # pl_check_term以上のpl logが溜まったらcheckを開始
            if np.gradient(ta.MA(np.array(ac2.total_pl_log[-avert_period_kijun:], dtype='f8'), timeperiod=avert_period_kijun))[-1] > avert_val_kijun:
                if ac2.holding_side != ac.holding_side and ac2.holding_side != '':
                    ac.entry_order(ac2.holding_side, TickData.get_bid() if ac2.posi_side == 'Buy' else TickData.get_ask(), ac2.holding_size, 'Limit')
                    print('sim onemin avert: Market order for position  {', ac.holding_side, ' x ', ac.holding_size, ' @', ac.holding_price + '}')
                elif ac2.order_side != ac.order_side:
                    if ac.order_side != '':
                        ac.cancel_all_orders()
                    ac.entry_order(ac2.order_side, ac2.price, ac2.size, ac2.order_side, TickData.get_bid() if ac2.order_side == 'Buy' else TickData.get_ask(), datetime.now())
                    print('sim onemin avert:' + ac2.order_type + ' entry order {', ac2.orderside, ' x ', ac.holding_size, ' @', ac.holding_price + '}')
        else:  # logがたまるまでは普通にトレード
            ac = cls.sim_model_pred_onemin_limit_entry(pred, pt_ratio, lc_ratio, amount, ltp, ac, ohlc)
        ac.check_execution(ltp, datetime.now(), ohlc)
        ac.update_pnl(ltp)
        ac2.check_execution(ltp, datetime.now(), ohlc)
        ac2.update_pnl(ltp)
